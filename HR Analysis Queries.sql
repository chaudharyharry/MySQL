...
SELECT DEPARTMENT, (CONCAT(ROUND(COUNT(ATTRITION)/(SELECT COUNT(*) FROM HR_1 WHERE ATTRITION="YES")*100,2), "%")) AS ATTRITION_RATE
FROM HR_1
WHERE ATTRITION="YES"
GROUP BY DEPARTMENT;

/* MONTHLY INCOME VS ATTRITION RATE BYU DEPARTMENT */
SELECT DEPARTMENT, 
	  (CONCAT(ROUND(COUNT(ATTRITION)/(SELECT COUNT(*) FROM HR_1 WHERE ATTRITION="YES")*100,2), "%")) AS ATTRITION_RATE,
       AVG(`MonthlyIncome`)
FROM HR_1 JOIN HR_2 
